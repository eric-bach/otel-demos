...
// acquire meter
const meter = metrics.getMeter('js-sample-app-meter');
let testingId = '';
...
totalApiRequestsMetric.addCallback((measurement) => {
  measurement.observe(totalApiRequests, commmon_attributes);
});
...
function updateTotalBytesSent(bytes: number, apiName: string, statusCode: number): void {
...
function updateLatencyTime(returnTime: number, apiName: string, statusCode: number): void {
...
function updateApiRequestsMetric(): void {
  totalApiRequests += 1;
  console.log('API Requests:' + totalApiRequests);
}

export { updateLatencyTime, updateTotalBytesSent, updateApiRequestsMetric };
